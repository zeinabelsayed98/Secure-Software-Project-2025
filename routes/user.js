...
module.exports = (app,db) => {
  ...
  .then(user => {
    ...
    */
     ...
     * @tags user
     * @param {User} request.body.required - update credentials
     * @param {integer} user_id.path.required
     * @return {string} 200 - success
     */
     app.put('/v1/user/:id', (req,res) =>{

        // ✅ السطر ده كان ناقص أو مكتوب غلط، وهو اللي بيجيب الـ ID من الرابط
        const userId = req.params.id;
        
        // الكود الآمن (تحديد الحقول المسموحة)
        db.user.update(
            {
                name: req.body.name,
                ...
                // role: req.body.role // ممنوع هنا
            }, 
            {
                where: {
                    id : userId // دلوقتي الكود هيشوف المتغير ده صح
                }
            }
        )
        .then((result)=>{
            // النتيجة بترجع مصفوفة فيها عدد الصفوف المعدلة
            res.json(result); 
        })
        .catch(err => {
            console.log(err);
